-- https://github.com/vladimir-g/fainty/blob/master/widgets/pulseaudio.lua
...
function PulseChannel:raise(value)
    ...
    os.execute("pactl set-" .. self.channel_type .. "-volume " ..
                  self.name .. ' +' .. value .. '%')
end
...
function PulseChannel:toggle()
    os.execute("pactl set-" .. self.channel_type .. "-mute " ..
                  self.name .. " toggle")
end
...
local function new(args)
    ...
    -- Set settings
    obj.channels = {}
    obj.format = args.settings.format or "%s %d %% "
